...
import { All } from './entities/all.entity';
...
export class AllService {
  ...
  //   return await this.allRepository.findOne({ where: { subject_code } });
  // }

  //^ GET
  async findAll(): Promise<All[]> {
    return await this.allRepository.find();
  }
  ...
  //^ POST
  async create(createFirstDto: CreateFirstDto): Promise<All> {
    const newSchedule = this.allRepository.create({ ...createFirstDto });
    return await this.allRepository.save(newSchedule);
  }
  ...
  async update(id: number, updateDto: UpdateFirstDto): Promise<void> {
    ...
    const existingFirst = await this.allRepository.findOne({
      where: { id },
    ...
    await this.allRepository.save(existingFirst);
  ...
  async delete(id: number): Promise<void> {
    ...
    const result = await this.allRepository.delete(id);
